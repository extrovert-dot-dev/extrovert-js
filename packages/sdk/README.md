...
Submit limited evidence with `feedback.submit`, open a tracked conversation with `support.cases.create`, and follow published replies with `support.cases.get` / `.events`. MCP users can use `submit_feedback` and the support case tools. New agent presets include `support:submit` to file and follow their own feedback and cases. Select `support:read` / `support:write` deliberately for other reports within the authorized project or organization; project managers include all three. An explicit credential `scopes` list replaces defaults, so omit support scopes to opt out. Automatic reporting remains opt-in and Assistant reporting is explicit-only. See [support reporting](https://docs.extrovert.dev/operating/support/) and the `extrovert-support` skill.
