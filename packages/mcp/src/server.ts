...
const INSTRUCTIONS = [
  ...
  "",
  "Typical flow:",
  "  1. sign_up then verify_signup when started with no key, or redeem_enrollment when given an enrollment token.",
  "  2. create_inbox — mint another inbox (omit username/domain for an instant shared-subdomain address).",
  ...
  "  5. read_messages / get_message / list_threads / search — inspect the inbox directly; do not build raw HTTP or jq helpers.",
  "",
  "The packaged local stdio server stores full keys after verify_signup or redeem_enrollment and reloads them in future sessions.",
